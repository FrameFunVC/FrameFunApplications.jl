...
# sol = _run_L_example(;opts...)

# using Plots
# heatmap_plot(200,sol.u,sol.domain)
# quiver_plot(25,sol.u,sol.domain;γ=5)
#
#
#
#
# EquispacedGrid()
# (δx^0*δy)(sol.u)(sol.flow_grid)
#
#
# heatmap_plot(200,(δx^0*δy)(sol.u),sol.domain)
# (x,y),((Vx,Vy),V) = velocity_matrices(sol.u,sol.domain,sol.innergrid)
# heatmap(x,y,Vx)
# heatmap(x,y,Vy)
# heatmap(x,y,V)
# norm(Vx[:,end],Inf)
...
#     # basis = Fourier(N)^2
#     basis = BSplineTranslatesBasis(N,3)^2
#     innerbox = (prevfloat(a)..nextfloat(b))×(prevfloat(c)..nextfloat(d))
#     _disk = r*disk().+[.5,.5]
...
#     @info "testing solution"
#     @show norm((δx^0*δy)(u)(flow_grid) .- v,Inf)
#     @show norm((δx^1*δy^0)(u)(flow_grid),Inf)
#     @show norm((δx*δy^0)(u)(homogeneous_grid),Inf)
#     @show norm(Δ(u)(collocation_grid),Inf)
#     _circle_grid = circle_grid(r,γ*N)
#     _circle_velocity_vectors = velocity_vectors(u,_circle_grid)
#     _circle_normal = normal.(_circle_grid,Ref(domain))
#     _normal_velocity = map((nxy,dx,dy)->nxy[1]*dx+nxy[2]*dy
#         ,_circle_normal,_circle_velocity_vectors[1],_circle_velocity_vectors[2])
#     @show norm(_normal_velocity,Inf)
#     @info "============================="
#     u, domain
# end
# u,D = circle_example(r=.15,N=100,γ=2)
#
# using Plots
# heatmap_plot(200,u,D;clims=(-.2,1.5))
# quiver_plot(21,u,D;γ=8)
#
#
# Vx,Vy,V = velocity_matrices(u,D,EquispacedGrid(8*21,.2,.8)^2;fill=0.)
# heatmap(V;layout=(1,3),subplot=2,ratio=1,size=(600,200))
#     heatmap!(Vx,subplot=1,ratio=1)
#     heatmap!(Vy,subplot=3,ratio=1)
#
#
# norm(V[1,:],Inf)
...
# element(coefficients(sol.u),2)
#
# sol = FrameFunApplications.SplineSquareExample._run_quarter_square_example(;
#     N=400,γ=4,r=.3,verbose=true)
#
#
# sol = FrameFunApplications.SplineSquareExample._run_quarter_square_example_sing(;
#     N=100,γ=2,r=.17,verbose=true,sing=false)
#
# heatmap_plot(100,sol.u,sol.domain)
# quiver_plot(21,sol.u,sol.domain)
# @show element(coefficients(sol.u),2)
# @show norm(coefficients(sol.u))
#
# f = identity
# R = 1
# xc = -.1
# yc = -.1
# f = z->z+(sqrt(R^2-yc^2)-xc)^2/z
# # f = identity
# scatter(f.(xc.+1im*yc.+cis.(LinRange(0,2pi,100))),
#     ratio=1)
